...
d0 = hipc_merged_young_noNorm_filtered %>%
  filter(study_time_collected == 0) %>%
  select(participant_id, any_of(gene_names))

d1 = hipc_merged_young_noNorm_filtered %>%
  filter(study_time_collected == 1) %>%
  select(participant_id, any_of(gene_names))

d3 = hipc_merged_young_noNorm_filtered %>%
...
sources <- list(
  d0   = list(df = hipc_merged_young_noNorm_filtered, tp = 0L),
  d1   = list(df = hipc_merged_young_noNorm_filtered, tp = 1L),
  d3   = list(df = hipc_merged_young_noNorm_filtered, tp = 3L),
