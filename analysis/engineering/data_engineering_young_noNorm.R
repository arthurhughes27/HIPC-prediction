...
engineer_expression_data <- function(study_of_interest,
  ...
  p_load_expr_young_noNorm <- fs::path(processed_data_folder, "hipc_merged_young_noNorm.rds")
  p_load_btm           <- fs::path(processed_data_folder, "BTM_processed.rds")
  
  # Load data objects
  hipc_merged_young_noNorm <- readRDS(p_load_expr_young_noNorm)
  ...
  hipc_merged_young_noNorm_filtered <- hipc_merged_young_noNorm %>%
    filter(
  ...
  gene_names <- hipc_merged_young_noNorm_filtered %>%
    dplyr::select(a1cf:zzz3) %>%
    dplyr::select(where( ~ !any(is.na(.)))) %>%
    colnames()
  
  
  # df = hipc_merged_young_noNorm_filtered
  ...
  # mapping of top-level datasets to source data and timepoint (generalized)
  sources <- list()
  for (tp in timepoints_of_interest) {
    top_name <- paste0("d", tp)
    sources[[top_name]] <- list(df = hipc_merged_young_noNorm_filtered, tp = as.integer(tp))
  }
  
  # precompute fold-change tables for non-zero timepoints and add them as separate sources
  nonzero_tps <- timepoints_of_interest[timepoints_of_interest != 0]
  for (tp in nonzero_tps) {
    dfc <- compute_gene_fc(
      df = hipc_merged_young_noNorm_filtered,
      id_col = id_col,
      time_col = time_col,
    ...
    dfc[[time_col]] <- as.integer(tp)
    top_name_fc <- paste0("d", tp, "fc")
    sources[[top_name_fc]] <- list(df = dfc, tp = as.integer(tp))
  }
  ...
  p_save <- fs::path(
    ...
    paste0(
      "engineered_dataframes_influenzain_young_noNorm_",
