for (assay_of_interest in c("nAb", "hai")) {
  for (gender_of_interest in c("none", "Male", "Female")) {
    for (model_of_interest in c("elasticnet")) {
      ...
      sapply(list.files("R/", pattern = "\\.R$", full.names = TRUE),
             source)
      ...
      df.clinical.full = readRDS(df.clinical.path) %>%
        mutate(
      ...
      file_name = paste0(
        ...
        "_",
        model_of_interest,
        "_varianceFiltration.rds"
      )
      
      p_save <- fs::path(output_folder, file_name)
      
      # Skip if results already exist
      if (fs::file_exists(p_save)) {
        message(sprintf("Skipping existing result: %s", file_name))
        next
      }
      
      # Inside the innermost outer loops, before looping over data.sel / feat.eng.col / feat.eng.row
      df.clinical <- df.clinical.full
      
      # Filter by gender
      if (gender_of_interest == "Male") {
        df.clinical <- df.clinical %>% filter(genderMale == 1)
      } else if (gender_of_interest == "Female") {
        df.clinical <- df.clinical %>% filter(genderMale == 0)
      }
      ...
      # Generate new folds for this subset
      fold_df <- balance_folds(
        df = pid_df,
        ind.col = "participant_id",
        covariate.cols = c(response.col),
      ...
      # Use these folds in your cv.predict call
      fold.ids <- fold_df$fold
      ...
      for (data.sel in names(df.predictor.list)) {
        for (feat.eng.col in names(df.predictor.list[[data.sel]])) {
          for (feat.eng.row in names(df.predictor.list[[data.sel]][[feat.eng.col]])) {
            ...
            # Perform variable selection on the entire dataset 
            ...
            res = feature.selection.variance(
              df = df_pred,
