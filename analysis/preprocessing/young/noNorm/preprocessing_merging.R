...
hipc_merged_clinical_immresp_young_noNorm = full_join(x = hipc_clinical_young_noNorm, y = hipc_immResp, by = c("participant_id", "study_accession"))


# Take distinct rows with respect to participant id and timepoint

hipc_merged_clinical_immresp_young_noNorm  = hipc_merged_clinical_immresp_young_noNorm %>%
...
hipc_merged_young_noNorm = right_join(
  x = hipc_merged_clinical_immresp_young_noNorm,
  y = expr_young_noNorm,
...
p_save_clinical_immresp <- fs::path(processed_data_folder, "hipc_merged_clinical_immresp_young_noNorm.rds")

# Save dataframe
saveRDS(hipc_merged_young_noNorm, file = p_save_young_noNorm)
saveRDS(hipc_merged_clinical_immresp_young_noNorm, file = p_save_clinical_immresp)
