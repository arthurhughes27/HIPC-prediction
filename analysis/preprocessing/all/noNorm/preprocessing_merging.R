...
hipc_merged_clinical_immresp_all_noNorm = full_join(x = hipc_clinical_all_noNorm, y = hipc_immResp, by = c("participant_id", "study_accession"))


# Take distinct rows with respect to participant id and timepoint

hipc_merged_clinical_immresp_all_noNorm  = hipc_merged_clinical_immresp_all_noNorm %>%
...
hipc_merged_all_noNorm = right_join(
  x = hipc_merged_clinical_immresp_all_noNorm,
  y = expr_all_noNorm,
...
p_save_clinical_immresp <- fs::path(processed_data_folder, "hipc_merged_clinical_immresp_all_noNorm.rds")

# Save dataframe
saveRDS(hipc_merged_all_noNorm, file = p_save_all_noNorm)
saveRDS(hipc_merged_clinical_immresp_all_noNorm, file = p_save_clinical_immresp)
